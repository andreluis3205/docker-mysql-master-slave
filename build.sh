...
until docker exec mysql_master sh -c 'export MYSQL_PWD=111; mysql -uroot -p111 -e ";"'
do
...
until docker-compose exec mysql_slave sh -c 'export MYSQL_PWD=111; mysql -uroot -p111 -e ";"'
do
    echo "Waiting for mysql_slave database connection..."
    sleep 4
done

MS_STATUS=`docker exec mysql_master sh -c 'export MYSQL_PWD=111; mysql -uroot -p111 -e "SHOW MASTER STATUS"'`
...
start_slave_cmd='export MYSQL_PWD=111; mysql -uroot -p111 -e "'
start_slave_cmd+="$start_slave_stmt"
start_slave_cmd+='"'
docker exec mysql_slave sh -c "$start_slave_cmd"

docker exec mysql_slave sh -c "export MYSQL_PWD=111; mysql -uroot -p111 -e 'SHOW SLAVE STATUS \G'"
